...
survey_lung_cancer2_ <- read_excel("E:/survey lung cancer2..xlsx")
...
ggplot(data=df2, aes(x=GENDER, y=`AgeofSmokers`)) +
  ...
  labs(title = 'LUNGS CANCER COMPARISION OF PEOPLE SMOKING', 
...
LRegression.data<-glm(formula=LUNG_CANCER~GENDER+AGE+ALCOHOL_CONSUMING, family = binomial)
LRegression.data
plot(LRegression.data)

LRegression.data<-glm(formula=LUNG_CANCER~GENDER+AGE+SMOKING, family = binomial)
LRegression.data
plot(LRegression.data)

LRegression.data<-glm(formula=LUNG_CANCER~GENDER+AGE+SMOKING+ALCOHOL_CONSUMING, family = binomial)
LRegression.data
plot(LRegression.data)
